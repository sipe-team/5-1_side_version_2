...
### 2026.06.23 현재까지 작업 상황 보고

- [이미지 컴포넌트 디자인 협업 범위 검토](https://minjeeki.notion.site/388196bee1fc805e8bade0c59d5004b9?source=copy_link) 관련 작업 계획 문서화. 아직 정민님과 논의를 진행하지 못함.

- 지훈님의 추가 코멘트에 따라 onLoad 관련 부분 역시 노출될 수 있도록 수정. 아직 PR에 올리지 않음.

- 커스텀 훅 서브패스 분리는 현재 어떻게 진행해야 하나 고민 중.. 작업은 하고 있긴 한데.. 다다음주 회의 전까지 진행할 예정..

### sprint 1 (2026.07.13)

#### 진행 내용

- image 컴포넌트 잔업(지훈님께서 추가로 제안해주신 onLoad 콜백 추가) 진행
- 정민님과 1차 기간 내 토큰 관련 간단하게 논의 후 radius에 대해서 우선적으로 토큰 적용하기로 결정한 후 token 적용 완료
- 훅 관련해서는 추가적인 패키지 분리이기 때문에 코드 작업은 1차적으로 일전에 해뒀으나 이미지 컴포넌트 머지 이후 받아서 작업하려고 보류해둔 상태
- 이미지 컴포넌트 이외 담당 컴포넌트 (input, checkbox, radio, switch) 작업을 위해 일전에 작성했던 개선 방향과 코드 다시 살펴봄

#### 다음 스프린트 전까지 업무 계획

- image 컴포넌트 머지 후 훅 관련 PR 업로드 후 검토 받기
- `sipe.team`에 이미지 컴포넌트 훅 패키지 적용
- 다른 담당 컴포넌트 관련 현재 토큰 활용 가능 케이스 분류 후 정민님과 추가 논의해서 만들어둔 토큰 적용할 수 있도록 작업